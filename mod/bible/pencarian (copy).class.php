...
class pencarian{
	...
	function frm_cari(){
		...
		$arr_surat 	= get_column("SELECT DISTINCT surat FROM bible ORDER BY nama_kitab,  surat ");
		...
		<table align='center' class='frm_cari' width='600px' border='0'>
			...
			<tr>
				...
				<td width='10px'>:</td> 
				<td>
					".autocomplete($_SESSION[surat]," size='60' ","txt_surat",$arr_surat)."
				</td>
			</tr>
			<tr>
				<td>Pasal</td><td>:</td>
	...
	function show(){
		...
		if($_SESSION[kitab] || $_SESSION[surat] || $_SESSION[isi] || $_SESSION[pasal] ){
			...
			$sql = "SELECT * FROM bible
				WHERE 
					...
					".($_SESSION[surat]?" AND MATCH(surat) AGAINST('{$_SESSION[surat]}') ":"")
					.($_SESSION[isi]?" AND MATCH(isi) AGAINST('{$_SESSION[isi]}') ":"")
				  . $syarat_pasal . $syarat_no_ayat ."
				  ORDER BY
				  ".($_SESSION[surat]?" MATCH(surat) AGAINST('{$_SESSION[surat]}') DESC ":"")
					.($_SESSION[surat]&&$_SESSION[isi]?",":"")
					.($_SESSION[isi]?"  MATCH(isi) AGAINST('{$_SESSION[isi]}') DESC ":"")
				  ." ,surat,pasal, no_ayat
		...
		foreach($table as $i => $row){
			...
			<div style='color:black;margin-top:10px;padding-left:30px;'>
				...
				<a class='ayat_bible' href='?mod=bible.pencarian.show&cari=1&cmb_kitab={$row[kitab]}&txt_surat={$row[surat]}&txt_pasal={$row[pasal]}'>
					&nbsp;	&nbsp;	&nbsp;	&nbsp; ".$this->tandai($row[isi])."
				</a>
			</div>
			";
